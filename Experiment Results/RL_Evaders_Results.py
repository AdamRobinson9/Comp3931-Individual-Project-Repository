...
class GameState:
    ...
    def reachable_teammates(self):
        ...
        # Return all teammates that can receive the flag
        return reachable
...
class CaptureTheFlag:
    ...
    def train(self, episode=0, pursuer_baseline=1, log=None):
        ...
        if log is not None:
            log.append({
                ...
                "epsilon": round(self.evader_controller.epsilon, 4)
            })

    def evaluate(self, episode=0, pursuer_baseline=1, log=None):
...
def get_pursuer_roles(baseline, num_pursuers):
    ...
    raise ValueError(f"Error: pursuer baseline {baseline}")

def get_evader_roles(num_evaders):
    """Starting roles for RL evaders — RL overrides these."""
...
def run_rl_experiments(output_csv="rl_evaders_results.csv", qtable_csv="rl_evaders_q_table.csv"):
    ...
    total = len(CONFIGURATIONS) * len(PURSUER_BASELINES)
    done = 0

    # Loop over all pursuer baselines
    for pursuer_baseline in PURSUER_BASELINES:
        # Loop over all different team sizes
        for (num_pursuers, num_evaders) in CONFIGURATIONS:
            ...
            # Initialise controller for RL, for evaders
            ...
            evader_roles = get_evader_roles(num_evaders)
            ...
            for episode in range(TRAIN_EPISODES):
                ...
                # Initialise evader agents
                for role in evader_roles:
                    start = Placement.choose_start_position(role, graph, pursuers, evaders)
                    evaders.append(Agent(start, role, is_pursuer=False))

                # Create game and run it to train policy
                game = CaptureTheFlag(graph, pursuers, evaders, flag_carrier=evaders[0], max_turns=MAX_TURNS, evader_controller=evader_controller)
                game.train(episode=episode, pursuer_baseline=pursuer_baseline, log=log)

            # Evaluation (epsilon=0, no Q updates)
            print("Evaluating: " + str(EVAL_EPISODES) + " episodes")
            evader_controller.epsilon = 0.0 # Set exploration to 0
            for episode in range(EVAL_EPISODES):
                ...
                game = CaptureTheFlag(graph, pursuers, evaders, flag_carrier=evaders[0], max_turns=MAX_TURNS, evader_controller=evader_controller)
                game.evaluate(episode=episode, pursuer_baseline=pursuer_baseline, log=log)

            # Store controller
            last_controller = evader_controller

    # Save episode log
    fields = ["episode", "phase", "pursuer_baseline", "n_pursuers", "n_evaders",
