...
class CaptureTheFlag:
    ...
    def train(self):
        ...
        while not self.state.game_over:
            ...
            for evader in self.state.evaders:
                if evader.d_before == None:
                    if evader.has_flag:
                        evader.d_before = min(nx.shortest_path_length(self.state.graph, evader.position, pursuer.position) for pursuer in self.state.pursuers)
                    else:
                        evader.d_before = nx.shortest_path_length(self.state.graph, evader.position, self.state.flag.position())
...
def main():
    ...
    num_pursuers = 6
    pursuer_roles = ["chase","chase","flank","flank","intercept","intercept","lurk","lurk"]
    num_evaders = 6
    evader_roles = ["escape", "support", "support", "support", "support", "support", "support", "support"]
    # Initialise shared RL controller for pursuer team
    ...
    for episode in range(200):
        ...
        game = CaptureTheFlag(graph, pursuers, evaders, flag_carrier, max_turns=200, pursuer_controller=pursuer_controller)
        game.train()
    ...
    pursuer_controller.epsilon = 0 # Exploitation only during play
