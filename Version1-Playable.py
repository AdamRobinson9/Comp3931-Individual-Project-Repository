...
class GameState:
    ...
    def check_flag_pass(self):
        ...
        # Get avaiable passes and prepare to print them
        candidates = self.reachable_teammates() 
        candidates_print = list(enumerate([candidate.position for candidate in candidates]))

        # Get the evader that the user wants to pass to
        pass_to = None
        while pass_to not in candidates:
            try:
                print("\nValid Passes: " + str(candidates_print))
                pass_to_ix = int(input("ENTER TEAMMATE TO PASS TO (Index): "))
                pass_to = candidates[pass_to_ix]
            except ValueError: # Ensure an integer is chosen
                print("Enter a valid integer. \n")
            except IndexError: # Ensure a valid teammate is chosen
                print("Choose an index from the list. \n")
        # Transfer flag to user's choice
...
class CaptureTheFlag:
    ...
    def __init__(self, graph, pursuers, evaders, flag_carrier, max_turns):
        ...
        self.layout = nx.kamada_kawai_layout(self.state.graph)
    ...
    def choose_move(self, agent):
        ...
        while new_position not in neighbours:
            try:
                ...
                if new_position not in neighbours: # must be a valid move
                    print("Choose a position from the valid moves list.\n")

            except ValueError: # Must be an integer
...
def choose_start_position(team, graph, evaders):
    """Get starting positions from the users for their agents"""
    print("TEAM:", team)
    ...
    while True:
        ...
        except ValueError: # Must be an integer to be a valid vertex
            print("Enter a valid integer. \n")

def draw_setup_graph(graph, pursuers, evaders):
    """Draw the graph during setup so users can see positions"""
    plt.clf()
